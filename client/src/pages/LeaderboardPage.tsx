...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    ...
    api.getWhatIfLeaderboard(gameId, selectedContestant)
      .then(setLeaderboard)
      .catch(err => setError(err instanceof Error ? err.message : 'Fout bij laden'));
    ...
    <div className="leaderboard-page">
      ...
      {isMoleRevealed ? (
        <div className="mole-revealed">
          🕵️ De Mol was: <strong>{game.contestants.find(c => c.id === game.moleContestantId)?.name}</strong>
        </div>
      ) : (
      ...
      {leaderboard.length > 0 ? (
        ...
        <p className="no-data">
          {isMoleRevealed ? 'Nog geen scores beschikbaar.' : 'Selecteer een kandidaat om het hypothetisch klassement te zien.'}
        </p>
      )}
    </div>
  );
